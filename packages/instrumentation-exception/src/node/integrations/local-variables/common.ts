...
export function isAnonymous(name: string | undefined): boolean {
  return (
    name !== undefined &&
    (name.length === 0 || name === '?' || name === '<anonymous>')
  );
}

/** Do the function names appear to match? */
export function functionNamesMatch(
  a: string | undefined,
  b: string | undefined,
): boolean {
...
export function hashFrames(
  ...
  return frames
    .slice(-10)
    .reduce(
      (acc, frame) => `${acc},${frame.function},${frame.lineno},${frame.colno}`,
      '',
    );
}
...
export interface LocalVariablesWorkerArgs
  extends LocalVariablesIntegrationOptions {
  /**
