...
class AsyncSession implements DebugSession {
  ...
  public static async create(
    orDefault?: DebugSession | undefined,
  ...
  public configureAndConnect(
    onPause: (event: OnPauseEvent, complete: () => void) => void,
    captureAll: boolean,
  ): void {
  ...
  private _getProperties(
    objectId: string,
    next: (result: Runtime.PropertyDescriptor[]) => void,
  ): void {
    this._session.post(
   ...
   */
  private _unrollArray(
    objectId: string,
    name: string,
    vars: Variables,
    next: (vars: Variables) => void,
  ): void {
    this._getProperties(objectId, (props) => {
      vars[name] = props
   ...
   */
  private _unrollObject(
    objectId: string,
    name: string,
    vars: Variables,
    next: (obj: Variables) => void,
  ): void {
    this._getProperties(objectId, (props) => {
      vars[name] = props
   ...
   */
  private _unrollOther(
    prop: Runtime.PropertyDescriptor,
    vars: Variables,
    next: (vars: Variables) => void,
  ): void {
    if (prop.value) {
...
const _localVariablesSyncIntegration = ((
  ...
  function addLocalVariablesToException(exception: Exception): void {
    ...
    const frames = (exception.stacktrace?.frames || []).filter(
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      if (
        ...
        !functionNamesMatch(
          frames[frameIndex].function,
          cachedFrame[i].function,
        )
      ) {
        continue;
      }
      ...
      AsyncSession.create(sessionOverride).then(
        (session) => {
          function handlePaused(
            ...
            }: InspectorNotification<PausedExceptionEvent>,
            complete: () => void,
          ): void {
            ...
            const { add, next } = createCallbackList<FrameVariables[]>(
              (frames) => {
          ...
          session.configureAndConnect(
            (ev, complete) =>
              handlePaused(
                defaultStackParser,
                ev as InspectorNotification<PausedExceptionEvent>,
                complete,
              ),
            captureAll,
          );
        ...
        (error) => {
          logger.log(
            'The `LocalVariables` integration failed to start.',
            error,
          );
        },
      );
 ...
 * Adds local variables to exception frames.
 */
export const localVariablesSyncIntegration = defineIntegration(
  _localVariablesSyncIntegration,
);
