...
const stackParser = createStackParser(
  nodeStackLineParser(createGetModuleFromFilename(options.basePath)),
);
...
async function unrollArray(
  session: Session,
  objectId: string,
  name: string,
  vars: Variables,
): Promise<void> {
  const properties: Runtime.GetPropertiesReturnType = await session.post(
...
async function startDebugger(): Promise<void> {
  ...
  session.on(
    'Debugger.paused',
    (event: InspectorNotification<Debugger.PausedEventDataType>) => {
      isPaused = true;

      handlePaused(
        session,
        stackParser,
        event.params as PausedExceptionEvent,
      ).then(
        () => {
  ...
  if (captureAll) {
    ...
    rateLimiter = createRateLimiter(
      ...
      async (seconds) => {
        log(
          `Rate-limit exceeded. Disabling capturing of caught exceptions for ${seconds} seconds.`,
        );
        await session.post('Debugger.setPauseOnExceptions', {
          state: 'uncaught',
        });
