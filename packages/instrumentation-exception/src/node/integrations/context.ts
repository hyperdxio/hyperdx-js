...
export const _nodeContextIntegration = ((options: ContextOptions = {}) => {
 ...
 */
export const nodeContextIntegration = defineIntegration(
...
function _updateContext(contexts: Contexts): Contexts {
  ...
  if (
    ...
    typeof (process as ProcessWithCurrentValues).availableMemory === 'function'
  ) {
    const freeMemory = (
      process as ProcessWithCurrentValues
    ).availableMemory?.();
    if (freeMemory != null) {
...
export function getDeviceContext(
  deviceOpt: DeviceContextOptions | true,
): DeviceContext {
...
async function getLinuxInfo(): Promise<OsContext> {
  ...
  try {
    ...
    const contents = (
      (await readFileAsync(distroPath, { encoding: 'utf-8' })) as string
    ).toLowerCase();
