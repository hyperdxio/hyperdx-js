  ...
  return {
    ...
    setup(client) {
      ...
      client.on('beforeEnvelope', (envelope): void => {
        ...
        const profiledTransactionEvents =
          findProfiledTransactionsFromEnvelope(envelope);
        ...
        for (const profiledTransaction of profiledTransactionEvents) {
          ...
          const profile_id =
            context && context['profile'] && context['profile']['profile_id'];
          const start_timestamp =
            context &&
            context['profile'] &&
            context['profile']['start_timestamp'];
          ...
          if (!profile) {
            diag.debug(
              `[Profiling] Could not retrieve profile for span: ${profile_id}`,
            );
            continue;
          }
...
export const browserProfilingIntegration = defineIntegration(
