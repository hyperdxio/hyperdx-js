...
function isUserAgentData(data: unknown): data is UserAgentData {
  return (
    typeof data === 'object' && data !== null && 'getHighEntropyValues' in data
  );
}
...
function isProcessedJSSelfProfile(
  profile: ThreadCpuProfile | JSSelfProfile,
): profile is JSSelfProfile {
...
function getTraceId(event: Event): string {
  ...
  if (typeof traceId === 'string' && traceId.length !== 32) {
    diag.debug(`[Profiling] Invalid traceId: ${traceId} on profiled event`);
  }
    ...
    transactions: [
      {
        ...
        relative_start_ns: '0',
        relative_end_ns: (
          (transactionEndMs - transactionStartMs) *
          1e6
        ).toFixed(0),
      },
    ],
 ...
 *
 */
export function isProfiledTransactionEvent(
  event: Event,
): event is ProfiledEvent {
  return !!(
    event.sdkProcessingMetadata && event.sdkProcessingMetadata['profile']
  );
}
 ...
 */
export function convertJSSelfProfileToSampledFormat(
  input: JSSelfProfile,
): Profile['profile'] {
  ...
  const origin =
    typeof performance.timeOrigin === 'number'
      ...
      : browserPerformanceTimeOrigin || 0;
  const adjustForOriginChange =
    origin - (browserPerformanceTimeOrigin || origin);
  ...
  for (let i = 0; i < input.samples.length; i++) {
    ...
    if (jsSample.stackId === undefined) {
      ...
      profile['samples'][i] = {
        ...
        elapsed_since_start_ns: (
          (jsSample.timestamp + adjustForOriginChange - start) *
          MS_TO_NS
        ).toFixed(0),
    ...
    let stackTop: JSSelfProfileStack | undefined =
      input.stacks[jsSample.stackId];
    ...
    while (stackTop) {
      ...
      if (profile.frames[stackTop.frameId] === undefined) {
        profile.frames[stackTop.frameId] = {
          function: frame.name,
          abs_path:
            typeof frame.resourceId === 'number'
              ? input.resources[frame.resourceId]
              : undefined,
          lineno: frame.line,
          colno: frame.column,
        };
      ...
      stackTop =
        stackTop.parentId === undefined
          ? undefined
          : input.stacks[stackTop.parentId];
    }
 ...
 * @param envelope
 */
export function addProfilesToEnvelope(
  ...
  forEachEnvelopeItem(envelope, (item, type) => {
    ...
    for (let j = 1; j < item.length; j++) {
      ...
      if (
        event &&
        event.contexts &&
        event.contexts['profile'] &&
        event.contexts['profile']['profile_id']
      ) {
 ...
 */
export function applyDebugMetadata(
...
export function isValidSampleRate(rate: unknown): boolean {
  ...
  if (rate < 0 || rate > 1) {
    diag.warn(
      `[Profiling] Invalid sample rate. Sample rate must be between 0 and 1. Got ${rate}.`,
    );
    return false;
  }
  return true;
...
function isValidProfile(
  profile: JSSelfProfile,
): profile is JSSelfProfile & { profile_id: string } {
  ...
  if (!profile.frames.length) {
    diag.debug('[Profiling] Discarding profile because it contains no frames');
    return false;
  }
 ...
 * @param maybeProfiler
 */
function isJSProfilerSupported(
  maybeProfiler: unknown,
): maybeProfiler is typeof JSSelfProfilerConstructor {
...
export function startJSSelfProfile(): JSSelfProfiler | undefined {
  ...
  try {
    ...
    diag.debug('[Profiling] Disabling profiling for current user session.');
...
export function shouldProfileSpan(span: Span): boolean {
  ...
  const options = client && client.getOptions();
  if (!options) {
    diag.debug('[Profiling] Profiling disabled, no options found.');
    return false;
  }

  // TODO: fix type ?
  const profilesSampleRate: number | boolean | undefined = (options as any)
    .profilesSampleRate;

  // Since this is coming from the user (or from a function provided by the user), who knows what we might get. (The
  // only valid values are booleans or numbers between 0 and 1.)
  if (!isValidSampleRate(profilesSampleRate)) {
    diag.warn('[Profiling] Discarding profile because of invalid sample rate.');
    return false;
  }
  ...
  const sampled =
    profilesSampleRate === true ? true : Math.random() < profilesSampleRate;
 ...
 */
export function takeProfileFromGlobalCache(
  profile_id: string,
): JSSelfProfile | undefined {
 ...
 */
export function addProfileToGlobalCache(
  profile_id: string,
  profile: JSSelfProfile,
): void {
