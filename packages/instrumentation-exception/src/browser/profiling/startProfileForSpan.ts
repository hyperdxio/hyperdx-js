...
export function startProfileForSpan(span: Span): void {
  ...
  diag.debug(
    `[Profiling] started profiling span: ${spanToJSON(span).description}`,
  );
  ...
  async function onProfileHandler(): Promise<void> {
    ...
    return profiler
      ...
      .then((profile: JSSelfProfile): void => {
        ...
        diag.debug(
          `[Profiling] stopped profiling of span: ${
            spanToJSON(span).description
          }`,
        );
      ...
      .catch((error) => {
        diag.debug('[Profiling] error while stopping profiler:', error);
      });
  }
