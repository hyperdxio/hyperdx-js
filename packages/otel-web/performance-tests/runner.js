...
async function run(options) {
  ...
  const chrome = await chromeLauncher.launch({
    chromeFlags: [
      ...
      options.disableProxy
        ? ''
        : `--proxy-server=http://localhost:${injectingProxy.port}`,
    ],
  });
  ...
  console.log(
    '\tEstimated FID',
    report.audits['estimated-input-latency'].displayValue,
  );
  console.log(
    '\tFirst paint',
    report.audits['first-meaningful-paint'].displayValue,
  );
  ...
  if (bootupAudit) {
    console.log(
      '\tJS bootup time',
      bootupAudit.numericValue.toFixed(2) + ' ms',
    );
  }
  ...
  if (mainthreadAudit) {
    ...
    if (scriptParseCompileAudit) {
      console.log(
        '\tScript compilation',
        scriptParseCompileAudit.duration.toFixed(2) + 'ms',
      );
    }

    const scriptEvaluationAudit = mainthreadAudit.details.items.find(
      (item) => item.group === 'scriptEvaluation',
    );
    if (scriptEvaluationAudit) {
      console.log(
        '\tScript evaluation',
        scriptEvaluationAudit.duration.toFixed(2) + 'ms',
      );
    }
  }
