...
const REACT_DOM_LIB_URL =
  'https://unpkg.com/react-dom@17/umd/react-dom.production.min.js';

async function handleReactBundleRequest(app) {
  const reactPackage = await fetch(REACT_LIB_URL).then((response) =>
    response.text(),
  );
  const reactDomPackage = await fetch(REACT_DOM_LIB_URL).then((response) =>
    response.text(),
  );
  ...
  app.get(REACT_RUM_BUNDLE_PATH, (_, res) => {
    ...
    res.send(
      reactPackage + reactDomPackage + SimpleReactApp + splunkRumPackage,
    );
  });
}
