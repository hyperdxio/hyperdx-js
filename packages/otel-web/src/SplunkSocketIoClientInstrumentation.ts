...
interface SocketIOSocket {
  ...
  prototype: {
    ...
    ): ThisParameterType<SocketIOSocket>;
    addEventListener(
      ev: string,
      listener: (...args: unknown[]) => void,
    ): ThisParameterType<SocketIOSocket>;
...
export interface SocketIoClientInstrumentationConfig
...
function seemsLikeSocketIoClient(io: unknown): io is SocketIOClient {
  return (
    typeof io === 'function' &&
    typeof (io as SocketIOClient).Socket === 'function'
  );
}
...
export class SplunkSocketIoClientInstrumentation extends InstrumentationBase {
  ...
  enable(): void {
    ...
    if (typeof config.target === 'string') {
      this._onDisable = waitForGlobal(config.target, (io) =>
        this.patchSocketIo(io),
      );
    } else {
