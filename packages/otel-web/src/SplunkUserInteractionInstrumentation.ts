...
export interface SplunkUserInteractionInstrumentationConfig
  extends UserInteractionInstrumentationConfig {
  events?: UserInteractionEventsConfig;
}

function isPatchableEventListner(
...
export class SplunkUserInteractionInstrumentation extends UserInteractionInstrumentation {
  ...
  constructor(config: SplunkUserInteractionInstrumentationConfig = {}) {
    ...
    this._routingTracer = trace.getTracer(
      ROUTING_INSTRUMENTATION_NAME,
      ROUTING_INSTRUMENTATION_VERSION,
    );

    const _superCreateSpan = (this as unknown as ExposedSuper)._createSpan.bind(
      this,
  ...
  setTracerProvider(tracerProvider: TracerProvider): void {
    ...
    this._routingTracer = tracerProvider.getTracer(
      ROUTING_INSTRUMENTATION_NAME,
      ROUTING_INSTRUMENTATION_VERSION,
    );
  }
