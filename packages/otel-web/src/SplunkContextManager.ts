...
export class SplunkContextManager implements ContextManager {
  ...
  protected _getListenersMap(
    target: EventTarget,
    type: string,
  ): WeakMap<EventListener, EventListener> {
      ...
      (original) =>
        function (
          ...
          if (args[0] === 'hashchange' && isFunction(args[1])) {
            ...
            if (!wrapped) {
              wrapped = manager._getWrappedEventListener(
                args[1],
    ...
    wrapNatively(
      ...
      (original) =>
        function (this: Window, value) {
          if (isFunction(value)) {
            const wrapped =
              manager._getWrappedEventListener<EventListenerWithOrig>(
                value,
                () => manager._hashChangeContext,
              );
            wrapped._orig = value;
  ...
  protected _getWrappedEventListener<E extends EventListener>(
    orig: E,
    contextGetter: () => Context | void,
  ): E {
    // eslint-disable-next-line @typescript-eslint/no-this-alias
    const manager = this;
