...
const nodeHasSrcAttribute = (
  node: Node,
): node is HTMLScriptElement | HTMLImageElement =>
...
export class SplunkPostDocLoadResourceInstrumentation extends InstrumentationBase {
  ...
  constructor(config: SplunkPostDocLoadResourceInstrumentationConfig = {}) {
    const processedConfig: SplunkPostDocLoadResourceInstrumentationConfig =
      Object.assign(
        {},
    ...
    this.performanceObserver = new PerformanceObserver((list) => {
      if (window.document.readyState === 'complete') {
        list.getEntries().forEach((entry) => {
          ...
          if (
            this.config.allowedInitiatorTypes.includes(
