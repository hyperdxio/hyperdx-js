...
export class InternalEventTarget {
  ...
  addEventListener<T extends keyof RumOtelWebEventTypes>(
    type: T,
  ...
  removeEventListener<T extends keyof RumOtelWebEventTypes>(
    type: T,
    listener: SplunkEventListener<T>,
  ): void {
    if (!this.events[type]) {
  ...
  emit<T extends keyof RumOtelWebEventTypes>(
    type: T,
    payload: RumOtelWebEventTypes[T],
  ): void {
