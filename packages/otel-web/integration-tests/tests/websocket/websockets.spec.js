...
module.exports = {
  ...
  'produces correct connect span': async function (browser) {
    ...
    const wsConnectionSpan = await browser.globals.findSpan(
      (span) => span.name === 'connect',
      -1000,
    );

    await browser.assert.strictEqual(wsConnectionSpan.kind, 'CLIENT');
    await browser.assert.strictEqual(
      wsConnectionSpan.tags['location.href'],
      url,
    );
    await browser.assert.strictEqual(
      wsConnectionSpan.tags['app'],
      'splunk-otel-js-dummy-app',
    );
    await browser.assert.strictEqual(
      wsConnectionSpan.tags['component'],
      'websocket',
    );
  ...
  'websocket url can be ignored': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl('/websocket/websocket-ignored.ejs'),
    );
    await browser.click('#connectWs');

    await browser.globals.findSpan(
      (span) => span.name === 'websocket-guard-span',
    );
    await browser.assert.not.ok(
      browser.globals
        .getReceivedSpans()
        .find((span) => span.name === 'connect'),
    );
  },
  'sending send and on message create a span': async function (browser) {
    ...
    const wsMessage = await browser.globals.findSpan(
      (span) => span.name === 'onmessage',
    );
    ...
    await browser.assert.strictEqual(
      wsMessage.tags['http.url'],
      browser.globals.wsBase,
    );
    await browser.assert.strictEqual(
      wsMessage.tags['http.response_content_length'],
      '7',
    );

    const wsSend = browser.globals
      .getReceivedSpans()
      .find((span) => span.name === 'send');
    ...
    await browser.assert.strictEqual(
      wsSend.tags['http.url'],
      browser.globals.wsBase,
    );
    await browser.assert.strictEqual(
      wsSend.tags['http.request_content_length'],
      '12',
    );
  },
  'websocket constructor errors are captured': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl('/websocket/websocket-construct-errors.ejs'),
    );
    await browser.click('#connectWs');
    const wsConnectionSpan = await browser.globals.findSpan(
      (span) => span.name === 'connect',
  ...
  'specifying sub-protocols does not break anything': async function (browser) {
    ...
    await browser.assert.ok(!!wsConnectionSpan, 'Connect span missing');
    await browser.assert.strictEqual(
      wsConnectionSpan.tags['protocols'],
      '["soap"]',
    );
