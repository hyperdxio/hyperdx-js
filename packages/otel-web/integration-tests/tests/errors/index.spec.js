...
module.exports = {
  ...
  'manual console.error': async function (browser) {
    ...
    const ERROR_MESSAGE_MAP = {
      safari:
        "null is not an object (evaluating 'someNull.anyField = 'value'')",
      ...
      'internet explorer':
        "Unable to set property 'anyField' of undefined or null reference",
    };
    await browser.assert.strictEqual(
      tags['error.message'],
      ERROR_MESSAGE_MAP[browserName],
    );
  ...
  'module can be disabled': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl(
        '/errors/views/unhandled-error.ejs?disableInstrumentation=errors',
      ),
    );
    await browser.globals.waitForTestToFinish();
