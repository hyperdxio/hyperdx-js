...
module.exports = {
  'traceId should be attached to documentFetch span if Server-Timing was sent':
    async function (browser) {
      ...
      const docFetchSpan = await browser.globals.findSpan(
        (span) =>
          ...
          span.tags['link.traceId'] === expectedTraceId,
      );

      await browser.assert.strictEqual(
        docFetchSpan.tags['link.traceId'],
        expectedTraceId,
      );
      await browser.assert.strictEqual(
      ...
      await browser.assert.strictEqual(
        docFetchSpan.tags['app'],
        'splunk-otel-js-dummy-app',
      );
      await browser.assert.strictEqual(
        docFetchSpan.tags['component'],
        'document-load',
      );
      await browser.assert.strictEqual(
        docFetchSpan.tags['splunk.rumVersion'],
