...
module.exports = {
  ...
  'documentFetch, resourceFetch, and documentLoad spans': async function (
    browser,
  ) {
    const url = browser.globals.getUrl('/docload/docload.ejs');
    await browser.url(url);

    const docFetch = await browser.globals.findSpan(
      (span) => span.name === 'documentFetch',
    );
    const docLoad = await browser.globals.findSpan(
    ...
    await browser.assert.ok(
      docLoad.traceId.match(/[a-f0-9]+/),
      'Checking sanity of traceId',
    );
    await browser.assert.ok(
      docLoad.id.match(/[a-f0-9]+/),
      'Checking sanity of id',
    );
    ...
    if (!browser.globals.isBrowser({ safari: true, ie: true })) {
      ...
      await browser.assert.ok(
        docFetch.tags['link.traceId'],
        'Checking presence of link.traceId',
      );
      await browser.assert.ok(
        docFetch.tags['link.spanId'],
        'Checking presence of link.spanId',
      );
    }
  ...
  'ignoring resource URLs': async function (browser) {
    ...
    await browser.assert.not.ok(
      browser.globals
        ...
        .find(
          (span) =>
            span.tags['http.url'] === url + 'non-impactful-resource.jpg',
        ),
    );
  },
  'module can be disabled': async function (browser) {
    await browser.url(
      browser.globals.getUrl(
        '/docload/docload.ejs?disableInstrumentation=document',
      ),
    );
    ...
    await browser.assert.ok(
      await browser.globals.findSpan(
        (span) => span.name === 'eventListener.error',
      ),
    );
    ...
    await browser.assert.not.ok(
      browser.globals
        .getReceivedSpans()
        .find((span) => SPAN_TYPES.includes(span.name)),
