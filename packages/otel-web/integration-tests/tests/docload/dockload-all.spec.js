...
module.exports = {
  'resources before load event are correctly captured': async function (
    browser,
  ) {
    ...
    for (const urlEnd of resources) {
      const resourceSpan = receivedSpans.filter(
        (span) =>
          span.tags['http.url'] && span.tags['http.url'].endsWith(urlEnd),
      )[0];
      await browser.assert.ok(!!resourceSpan, urlEnd);
      await browser.assert.strictEqual(
        docLoad.traceId,
        resourceSpan.traceId,
        `${urlEnd} has correct traceId`,
      );
    }
