...
module.exports = {
  'handles keyboard events': async function (browser) {
    ...
    const keydownSpan = await browser.globals.findSpan(
      (span) => span.name === 'keydown',
    );
    await browser.assert.ok(!!keydownSpan, 'Checking keyboard span presence.');

    await browser.assert.strictEqual(
      keydownSpan.tags['component'],
      'user-interaction',
    );
    await browser.assert.strictEqual(keydownSpan.tags['event_type'], 'keydown');
    await browser.assert.strictEqual(
      keydownSpan.tags['target_element'],
      'BODY',
    );
    await browser.assert.strictEqual(
      keydownSpan.tags['target_xpath'],
      '//html/body',
    );

    const keyupSpan = await browser.globals.findSpan(
      (span) => span.name === 'keyup',
    );
    await browser.assert.ok(!!keyupSpan, 'Checking keyboard span presence.');

    await browser.assert.strictEqual(
      keyupSpan.tags['component'],
      'user-interaction',
    );
