...
module.exports = {
  'handles form submit': async function (browser) {
    ...
    const submitSpan = await browser.globals.findSpan(
      (span) => span.name === 'submit',
    );
    await browser.assert.ok(
      !!submitSpan,
      'Checking form submission span presence.',
    );

    await browser.assert.strictEqual(
      submitSpan.tags['component'],
      'user-interaction',
    );
