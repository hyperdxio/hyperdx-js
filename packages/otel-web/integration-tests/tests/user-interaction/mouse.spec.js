...
module.exports = {
  'handles mouse click': async function (browser) {
    ...
    await browser.assert.strictEqual(
      clickSpan.tags['component'],
      'user-interaction',
    );
    await browser.assert.strictEqual(clickSpan.tags['event_type'], 'click');
    await browser.assert.strictEqual(
      clickSpan.tags['target_element'],
      'BUTTON',
    );
    await browser.assert.strictEqual(
      clickSpan.tags['target_xpath'],
      '//*[@id="btn1"]',
    );
  ...
  'handles mouse down': async function (browser) {
    ...
    const mouseDownSpan = await browser.globals.findSpan(
      (span) => span.name === 'mousedown',
    );
    await browser.assert.ok(
      !!mouseDownSpan,
      'Checking mousedown span presence.',
    );

    await browser.assert.strictEqual(
      mouseDownSpan.tags['component'],
      'user-interaction',
    );
    await browser.assert.strictEqual(
      mouseDownSpan.tags['event_type'],
      'mousedown',
    );
    await browser.assert.strictEqual(
      mouseDownSpan.tags['target_element'],
      'BUTTON',
    );
    await browser.assert.strictEqual(
      mouseDownSpan.tags['target_xpath'],
      '//*[@id="btn1"]',
    );
  },
  'handles mouse up': async function (browser) {
    ...
    const mouseUpSpan = await browser.globals.findSpan(
      (span) => span.name === 'mouseup',
    );
    await browser.assert.ok(!!mouseUpSpan, 'Checking mouseup span presence.');

    await browser.assert.strictEqual(
      mouseUpSpan.tags['component'],
      'user-interaction',
    );
    await browser.assert.strictEqual(mouseUpSpan.tags['event_type'], 'mouseup');
    await browser.assert.strictEqual(
      mouseUpSpan.tags['target_element'],
      'BUTTON',
    );
    await browser.assert.strictEqual(
      mouseUpSpan.tags['target_xpath'],
      '//*[@id="btn1"]',
    );
  },
  'handles disabling of mouse events': async function (browser) {
    browser.globals.clearReceivedSpans();
    await browser.url(
      browser.globals.getUrl('/user-interaction/mouse-disabled.ejs'),
    );
    await browser.click('#btn1');
    await browser.click('#btnGuard');

    const guardSpan = await browser.globals.findSpan(
      (span) => span.name === 'guard',
    );
    ...
    await browser.assert.not.ok(
      browser.globals
        .getReceivedSpans()
        .find((span) => span.name === 'mouseup'),
      'Ensuring no mouseup span arrived.',
    );
    await browser.assert.not.ok(
      browser.globals
        .getReceivedSpans()
        .find((span) => span.name === 'mousedown'),
  ...
  'handles clicks with event listener on document': async function (browser) {
    browser.globals.clearReceivedSpans();
    await browser.url(
      browser.globals.getUrl('/user-interaction/mouse-document.ejs'),
    );
    await browser.click('#btn1');
  ...
  'this is bubbled event is correct (open-telemetry/opentelemetry-js-contrib#643)':
    async function (browser) {
      browser.globals.clearReceivedSpans();
      await browser.url(
        browser.globals.getUrl('/user-interaction/mouse-bubble.ejs'),
      );
