...
module.exports = {
  ...
  'handles causality of a mouse click': async function (browser) {
    await browser.url(
      browser.globals.getUrl('/user-interaction/causality.ejs'),
    );
    ...
    const clickSpan = await browser.globals.findSpan(
      (span) =>
        span.name === 'click' &&
        span.tags['target_xpath'] === '//*[@id="btn1"]',
    );
  ...
  'can be disabled': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl(
        '/user-interaction/causality.ejs?disableInstrumentation=interactions',
      ),
    );
    await browser.click('#btn1');
    await browser.globals.waitForTestToFinish();
