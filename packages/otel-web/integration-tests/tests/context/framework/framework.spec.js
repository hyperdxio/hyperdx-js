...
async function runTest(browser, filename) {
  ...
  const customSpan = await browser.globals.findSpan(
    (span) => span.name === 'custom-span',
  );
...
module.exports = {
  ...
  'React 17 with async': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl('/context/framework/react-latest.ejs'),
    );
    await browser.click('#btn1');
    await browser.globals.waitForTestToFinish();
    ...
    const clickSpans = browser.globals
      .getReceivedSpans()
      .filter((span) => span.name === 'click');
