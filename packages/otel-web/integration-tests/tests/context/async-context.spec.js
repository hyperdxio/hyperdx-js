...
async function runTest(browser, filename, extraChecks) {
  ...
  const childSpan = await browser.globals.findSpan(
    (span) => span.name === 'context-child',
  );

  browser.assert.not.ok(
    clickSpan.parentId,
    'Click span does not have a parent.',
  );
  console.log(childSpan);
  browser.assert.strictEqual(
    childSpan.parentId,
    clickSpan.id,
    'Child span belongs to user interaction trace.',
  );
...
module.exports = {
  ...
  setTimeout: async function (browser) {
    ...
    browser.assert.not.ok(
      childSpan.parentId,
      "Child span in a longer timeout doesn't have a parent",
    );
  ...
  'promise constructor': async function (browser) {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      browser.assert.not.ok(
        clickSpan.parentId,
        `Click ${i} span does not have a parent.`,
      );
      browser.assert.strictEqual(
        childSpan.parentId,
        clickSpan.id,
  ...
  'location - hashchange': async function (browser) {
    ...
    const routeChangeSpan = await browser.globals.findSpan(
