...
module.exports = {
  'webvitals spans': async function (browser) {
    ...
    const lcp = await browser.globals.findSpan(
      (span) => span.tags.lcp !== undefined,
    );
    const cls = await browser.globals.findSpan(
      (span) => span.tags.cls !== undefined,
    );
    const fid = await browser.globals.findSpan(
      (span) => span.tags.fid !== undefined,
    );
    const inp = await browser.globals.findSpan(
      (span) => span.tags.inp !== undefined,
    );
