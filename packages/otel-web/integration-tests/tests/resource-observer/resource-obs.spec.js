...
module.exports = {
  ...
  'should report resource loads happening after page load': async function (
    browser,
  ) {
    if (
      isBrowser(browser, {
    ...
    await browser.url(
      browser.globals.getUrl('/resource-observer/resources.ejs'),
    );
    ...
    const plAgentSpans = browser.globals
      ...
      .filter(
        (span) =>
          span.tags['http.url'] &&
          span.tags['http.url'].endsWith('splunk-otel-web.js'),
      );
    const plImageSpans = browser.globals
      ...
      .filter(
        (span) =>
          span.tags['http.url'] &&
          span.tags['http.url'].endsWith('no-cache.png'),
      );
    ...
    const scriptSpan = await browser.globals.findSpan(
      (span) =>
        span.tags['http.url'] && span.tags['http.url'].endsWith('test.js'),
    );

    await browser.assert.strictEqual(
      scriptSpan.tags['component'],
      'splunk-post-doc-load-resource',
    );
    const scriptUrl =
      browser.globals.getUrl('/utils/devServer/assets/', []) + 'test.js';
    await browser.assert.strictEqual(scriptSpan.tags['http.url'], scriptUrl);
    await browser.assert.strictEqual(
      scriptSpan.annotations.length,
      9,
      'Missing network events',
    );
  },
  'resources can be ignored': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl('/resource-observer/resources-ignored.ejs'),
    );
  ...
  'should create one span for cached resource': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl('/resource-observer/resources-twice.ejs'),
    );
    await browser.globals.findSpan((span) => span.name === 'guard-span');
  ...
  'should create two spans for non cached resource': async function (browser) {
    ...
    if (
      browser.options.desiredCapabilities.browserName.toLowerCase() ===
      'firefox'
    ) {
    ...
    await browser.url(
      browser.globals.getUrl('/resource-observer/resources-twice-no-cache.ejs'),
    );
    await browser.globals.findSpan((span) => span.name === 'guard-span');
    ...
    const docLoadImage = imageSpans.find(
      (span) => span.tags['component'] === 'document-load',
    );
    const afterLoadImage = imageSpans.find(
      (span) => span.tags['component'] === 'splunk-post-doc-load-resource',
    );

    await browser.assert.notEqual(docLoadImage.traceId, afterLoadImage.traceId);
  },
  'should propagate tracing context to created spans': async function (
    browser,
  ) {
    if (
      isBrowser(browser, {
    ...
    await browser.url(
      browser.globals.getUrl('/resource-observer/resources-custom-context.ejs'),
    );
    await browser.globals.findSpan((span) => span.name === 'guard-span');
    ...
    const plImageParentSpan = await browser.globals
      .getReceivedSpans()
      .find((span) => span.name === 'image-parent');
    ...
    await browser.assert.notEqual(
      plScriptRootSpan.traceId,
      plScriptParentSpan.traceId,
    );
    await browser.assert.not.ok(plScriptRootSpan.parentId);
    await browser.assert.equal(
      plScriptParentSpan.traceId,
      plScriptChildSpan.traceId,
    );
    await browser.assert.equal(
      plScriptChildSpan.parentId,
      plScriptParentSpan.id,
    );
  },
  "doesn't crash when postload instrumentation is disabled": async function (
    browser,
  ) {
    await browser.url(
      browser.globals.getUrl('/resource-observer/resources-custom-context.ejs'),
    ...
    const click = await browser.globals.findSpan(
      (span) => span.name === 'click',
    );
