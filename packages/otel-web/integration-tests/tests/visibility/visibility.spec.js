...
module.exports = {
  'visibility events are captured': async function (browser) {
    ...
    const hiddenSpans = browser.globals
      .getReceivedSpans()
      .filter((span) => span.name === 'visibility');
    await browser.assert.ok(
      !!tabHiddenSpan,
      'Visibility event for hidden tab exists',
    );
    await browser.assert.strictEqual(
      hiddenSpans.length,
      1,
      'There is only one visibility event',
    );
    ...
    const visibleSpans = browser.globals
      .getReceivedSpans()
      .filter((span) => span.name === 'visibility');
    await browser.assert.ok(
      !!tabVisibileSpan,
      'Visibility event for visible tab exists',
    );
    await browser.assert.strictEqual(
      visibleSpans.length,
      1,
