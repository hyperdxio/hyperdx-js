...
module.exports = {
  'applies Synthetics Run Id attribute on all spans': async function (browser) {
    ...
    await browser.assert.ok(
      guardSpan,
      'Ensuring guard-span has arrived before we start inspecting spans.',
    );

    for (const span of backendCtx.getReceivedSpans()) {
      await browser.assert.strictEqual(
        span.tags['Synthetics-RunId'],
        'abcd1234'.repeat(4),
      );
    }
  ...
  'does not apply Synthetics Run Id attribute on all spans if not included':
    async function (browser) {
      ...
      const guardSpan = await backendCtx.findSpan(
        (s) => s.name === 'guard-span',
      );
      await browser.assert.ok(
        guardSpan,
