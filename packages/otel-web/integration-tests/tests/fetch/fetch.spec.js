...
module.exports = {
  ...
  'span created for fetch includes all properties': async function (browser) {
    ...
    const fetchSpan = await browser.globals.findSpan(
      (span) =>
        span.tags['http.url'] && span.tags['http.url'].endsWith('/some-data'),
    );
    ...
    if (browser.options.desiredCapabilities.browserName !== 'Safari') {
      await browser.assert.strictEqual(
        fetchSpan.tags['http.response_content_length'],
        '49',
      );
    }
    ...
    await browser.assert.not.ok(
      browser.globals
        ...
        .find(
          (span) =>
            span.tags['http.url'] &&
            span.tags['http.url'].endsWith('/some-data'),
        ),
    );
    await browser.assert.not.ok(
      browser.globals
        ...
        .find(
          (span) =>
            span.tags['http.url'] &&
            span.tags['http.url'].endsWith('/no-server-timings'),
        ),
    );
  },
  'fetch reported over CORS': async function (browser) {
    ...
    await browser.url(
      backend2
        .getUrl('/fetch/fetch.ejs', undefined, {
          beaconPort: browser.globals.httpPort,
        })
        .toString(),
    );
    ...
    await browser.assert.strictEqual(fetchSpan.tags['http.method'], 'GET');
    await browser.assert.strictEqual(
      fetchSpan.tags['http.host'],
      `${backend2.httpHostname}:${backend2.httpPort}`,
    );

    await browser.globals.assertNoErrorSpans();
    await backend2.close();
  },
  'request body exists in request object (open-telemetry/opentelemetry-js#2411)':
    async function (browser) {
      await browser.url(browser.globals.getUrl('/fetch/fetch-post.ejs'));
      await browser.globals.findSpan(
        (span) =>
          span.tags['http.url'] && span.tags['http.url'].endsWith('/echo'),
      );
  ...
  'can be disabled (with xhr switch)': async function (browser) {
    ...
    await browser.url(
      browser.globals.getUrl(
        '/fetch/fetch.ejs?disableInstrumentation=xhr,fetch',
      ),
    );
    await browser.globals.waitForTestToFinish();
