...
module.exports = {
  ...
  'setting session cookie should work': async function (browser) {
    ...
    await browser.assert.notEqual(
      parent.tags['splunk.scriptInstance'],
      parent.tags['splunk.rumSessionId'],
    );

    const iframe = await browser.globals.findSpan(
      (span) =>
        span.name === 'documentFetch' &&
        span.tags['location.href'].includes('iframe.ejs'),
    );
    await browser.assert.ok(iframe.tags['splunk.rumSessionId']);
    await browser.assert.notEqual(
      iframe.tags['splunk.scriptInstance'],
      iframe.tags['splunk.rumSessionId'],
    );
  ...
  'setting session cookie in iframe should work': async function (browser) {
    ...
    if (
      !isBrowser(browser, 'internet explorer') &&
      browser.globals.enableHttps
    ) {
    ...
    if (!isBrowser(browser, { 'internet explorer': true, safari: true })) {
      await browser.assert.equal(
        cookie.sameSite,
        browser.globals.enableHttps ? 'None' : 'Strict',
      );
    }
  },
  'setting cookieDomain via config sets it on subdomains also': async function (
    browser,
  ) {
    ...
    */
    ...
    await browser.url(
      `${protocol}://127.0.0.1.nip.io:${browser.globals.httpPort}/cookies/cookies-domain.ejs`,
    );
    const cookie = await browser.getCookie('_splunk_rum_sid');
    await browser.assert.ok(cookie);

    await browser.url(
      `${protocol}://test.127.0.0.1.nip.io:${browser.globals.httpPort}/cookies/cookies-domain.ejs`,
    );
