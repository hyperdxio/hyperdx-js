...
module.exports = {
  ...
  'XHR request is registered': async function (browser) {
    ...
    await browser.assert.strictEqual(
      xhrSpan.tags['http.url'],
      browser.globals.getUrl('/some-data', []),
    );
    if (!isBrowser(browser, { safari: true, ie: true })) {
      await browser.assert.strictEqual(
        xhrSpan.tags['http.response_content_length'],
        '49',
      );
    }
  ...
  'module can be disabled': async function (browser) {
    ...
    browser.assert.ok(
      !!browser.globals
        .getReceivedSpans()
        .find((span) => span.tags['http.url'] === awaitedUrl),
    ...
    await browser.url(
      browser.globals.getUrl(
        '/xhr/views/xhr-basic.ejs?disableInstrumentation=xhr',
      ),
    );
    await browser.globals.waitForTestToFinish();
