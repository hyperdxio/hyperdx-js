...
} = require('../../utils/testBackendProvider');
...
async function buildBackendContext(browser) {
  ...
  const defaultTimeout = browser.globals.isBrowser({ safari: { max: 10 } })
    ? -10000
    : 0;
  ...
  return {
    ...
    findSpan: (testFn, timeout = defaultTimeout) =>
      findSpan(backend.getReceivedSpans(), testFn, timeout),
  ...
  beforeEach: async (browser, done) => {
    ...
    browser.timesMakeSense = async function (eventsArr, startName, endName) {
      ...
      await this.assert.ok(
        name2time[startName],
        `Checking presence of ${startName}`,
      );
      await this.assert.ok(
        name2time[endName],
        `Checking presence of ${endName}`,
      );
      await this.assert.ok(
