...
describe('can produce websocket events', () => {
  ...
  it('can produce a series of spans from basic usage', (done) => {
    ...
    const msgListener = function () {
      ...
      setTimeout(() => {
        const wsSpans = capturer.spans.filter(
          (span) => span.attributes.component === 'websocket',
        );
        ...
        assert.strictEqual(
          SpanKind.PRODUCER,
          wsSpans[1].kind,
          'Check second span kind.',
        );
        assert.strictEqual(
          12,
          wsSpans[1].attributes['http.request_content_length'],
        );
        ...
        assert.strictEqual(
          SpanKind.CONSUMER,
          wsSpans[2].kind,
          'Check third span kind.',
        );
        assert.strictEqual(
          8,
          wsSpans[2].attributes['http.response_content_length'],
        );
      ...
      'message',
      {
        handleEvent() {
          setTimeout(() => {
            ...
            assert.strictEqual(
              '["foo","bar"]',
              wsSpans[0].attributes.protocols,
            );
            assert.strictEqual('foo', wsSpans[1].attributes.protocol);
            socket.close();
            done();
  ...
  it.skip('can report failed connect to blocked port', () => {
    ...
    try {
      ...
      assert.strictEqual(
        'ws://127.0.0.1:53/',
        capturer.spans[0].attributes['http.url'],
      );
