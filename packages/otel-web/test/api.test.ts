...
describe('Transitive API', () => {
  ...
  describe('Span', () => {
    ...
    function subject() {
      return SplunkOtelWeb.provider!.getTracer('test').startSpan('test.span', {
        startTime,
      });
    }
    ...
    it('can set record an exception', () => {
      ...
      expect(
        spanCapturer.spans[0].events[0].attributes['exception.type'],
      ).to.eq('Error');
      expect(
        spanCapturer.spans[0].events[0].attributes['exception.message'],
      ).to.eq('Exception to be recorded.');
    });
  });
