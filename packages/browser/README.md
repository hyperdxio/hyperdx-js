...
If you need to fetch the API key from your backend (for example, to avoid exposing it in your client-side code), you can pass an async function to the `apiKey` option:
...
Note: When using an async function for `apiKey`, you should `await` the `init` call if you need to ensure HyperDX is fully initialized before proceeding.
