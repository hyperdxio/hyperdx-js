...
const USER_AGENT = 'HyperDX NodeJS';
...
class HyperdxLogger {
  constructor({
    ...
    /*
          Callback method executed on each bulk of messages sent to hyperdx.
  ...
  _setProtocol(port) {
    if (!protocolToPortMap[this.protocol]) {
      throw new Error(
        `Invalid protocol defined. Valid options are : ${JSON.stringify(
          Object.keys(protocolToPortMap),
        )}`,
      );
    }
  ...
  _sendMessagesUDP() {
    const udpSentCallback = (err) => {
      if (err) {
        ...
        this.callback(
          new Error(`Failed to send udp log message. err = ${err}`),
        );
      }
    };
  ...
  _addTimestamp(msg) {
    ...
    if (this.addTimestampWithNanoSecs) {
      ...
      msg['@timestamp_nano'] =
        msg['@timestamp_nano'] ||
        [now, time[1].toString().padStart(nanoSecDigits, '0')].join('-');
    }
  }
  ...
  log(msg, obj) {
    ...
    // FIXME: no need to attach type (only used for api param)
    // if (!msg.type) {
    //   msg.type = this.type;
    // }

    // FIXME: no need to attach timestamp
  ...
  _tryAgainIn(sleepTimeMs, bulk) {
    this._debug(
      `Bulk #${bulk.id} - Trying again in ${sleepTimeMs}[ms], attempt no. ${bulk.attemptNumber}`,
    );
    setTimeout(() => {
      this._send(bulk);
    }, sleepTimeMs);
        ...
        if (UNAVAILABLE_CODES.includes(errorCode)) {
          if (bulk.attemptNumber >= this.numberOfRetries) {
            return this.callback(
              new Error(
                `Failed after ${bulk.attemptNumber} retries on error = ${err}`,
              ),
              bulk,
            );
          }
...
const createLogger = (options) => {
  const l = new HyperdxLogger(options);
  l._timerSend();
  return l;
};
