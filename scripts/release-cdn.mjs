...
if (!process.env.CDN_DISTRIBUTION_ID) {
  throw new Error(
    'You are missing an environment variable CDN_DISTRIBUTION_ID.',
  );
}
...
const { data: releases, status } = await requestWithAuth(
  `GET /repos/${OWNER}/${REPO}/releases`,
);
if (status >= 400) {
  throw new Error(
    'There was an error while trying to fetch the list of releases.',
  );
}
...
console.log(
  `I have found the latest version to be: ${ghRelease.tag_name} named "${ghRelease.name}."`,
);

const { data: assets } = await requestWithAuth(
  `GET /repos/${OWNER}/${REPO}/releases/${ghRelease.id}/assets`,
);
console.log(
  `This release has ${assets.length} release artifacts: ${assets
    .map(({ name }) => name)
    .join(', ')}.`,
);
...
console.log(
  `This release will update following versions in CDN: ${versions
    .map(([version]) => version)
    .join(', ')}`,
);
...
for (const asset of assets) {
  ...
  const sha384Sum = crypto
    .createHash('sha384')
    .update(assetBuffer, 'utf-8')
    .digest('base64');
...
if (!isDryRun) {
  ...
  console.log(
    `Invalidation ${Invalidation.Id} sent. Typically it takes about 5 minutes to execute.`,
  );
  ...
  console.log(
    `Please verify that instructions are correct by navigating to: ${ghRelease.html_url}`,
  );
}
...
function generateScriptSnippet({
  ...
  if (isAutoUpdating) {
    ...
    lines.push(
      `<script src="${publicUrl}" integrity="${integrityValue}" crossorigin="anonymous"></script>`,
    );
  }
