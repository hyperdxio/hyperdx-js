...
getPackageRoots().forEach((packagePath) => {
  ...
  const hasMatchingInternalVersion =
    versionFileContent.indexOf(`export const VERSION = '${version}';`) > -1;
  if (!hasMatchingInternalVersion) {
    console.error(
      `The version in src/version.ts does not match the ${name} package version.`,
    );
    process.exitCode = 1;
  }
});
